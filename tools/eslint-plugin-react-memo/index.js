/*
 * Vendored copy of eslint-plugin-react-memo@0.0.3 (MIT, (c) Stefano J. Attardi).
 * Upstream: https://github.com/steadicat/eslint-plugin-react-memo
 *
 * Patched for ESLint 10, which removed the deprecated `context` members that
 * upstream relied on:
 *
 *   1. getIdentifierMemoStatus(): `context.getScope()` -> `context.sourceCode.getScope(node)`.
 *      The node is now threaded through as a parameter, because SourceCode#getScope
 *      needs the node instead of implicitly reading the current traversal node.
 *   2. `context.getFilename()` -> `context.filename` (2 call sites in checkFunction).
 *
 * These are the only edits; the rest is the published dist/index.js verbatim.
 * Verified to produce byte-identical lint output to the unpatched plugin on ESLint 8.
 */
'use strict';
    ...
    if (currentNode.type === "VariableDeclarator") {
        ...
        if (node.id !== null && componentNameRegex$1.test(node.id.name)) {
            ...
            if (context.filename === "<input>")
                return;
            var filename = path__namespace.basename(context.filename);
...
function getIdentifierMemoStatus(context, node) {
    var name = node.name;
    var variable = context.sourceCode.getScope(node).variables.find(function (v) { return v.name === name; });
...
var messages$1 = {
    "object-usememo-props": "Object literal should be wrapped in React.useMemo() if used as a prop",
    "object-usememo-deps": "Object literal should be wrapped in React.useMemo() if used as a hook dependency",
    "array-usememo-props": "Array literal should be wrapped in React.useMemo() if used as a prop",
    "array-usememo-deps": "Array literal should be wrapped in React.useMemo() if used as a hook dependency",
    "instance-usememo-props": "Object instantiation should be wrapped in React.useMemo() if used as a prop",
    "instance-usememo-deps": "Object instantiation should be wrapped in React.useMemo() if used as a hook dependency",
    "jsx-usememo-props": "JSX should be wrapped in React.useMemo() if used as a prop",
    "jsx-usememo-deps": "JSX should be wrapped in React.useMemo() if used as a hook dependency",
    "function-usecallback-props": "Function definition should be wrapped in React.useCallback() if used as a prop",
    "function-usecallback-deps": "Function definition should be wrapped in React.useCallback() if used as a hook dependency",
    "unknown-usememo-props": "Unknown value may need to be wrapped in React.useMemo() if used as a prop",
    "unknown-usememo-deps": "Unknown value may need to be wrapped in React.useMemo() if used as a hook dependency",
...
var messages = {
    "object-usememo-children": "Object literal should be wrapped in React.useMemo() if used as children",
    "array-usememo-children": "Array literal should be wrapped in React.useMemo() if used as children",
    "instance-usememo-children": "Object instantiation should be wrapped in React.useMemo() if used as children",
    "jsx-usememo-children": "JSX should be wrapped in React.useMemo() when used as children",
    "function-usecallback-children": "Function definition should be wrapped in React.useCallback() if used as children",
    "unknown-usememo-children": "Unknown value may need to be wrapped in React.useMemo() if used as children",
    "usememo-const": "useMemo/useCallback return value should be assigned to a const to prevent reassignment"
};
var rule = {
    meta: {
